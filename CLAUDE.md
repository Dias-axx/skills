...
Subagents live in `agents/` at the repo root, one Markdown file per agent, and ship with the plugin via `.claude-plugin/plugin.json`'s `agents` array — every file in `agents/` must be listed there, mirroring how `skills` is curated rather than auto-discovered. An agent's `skills:` frontmatter may only preload model-invoked skills (preloading a `disable-model-invocation: true` skill fails); route to user-invoked skills as advice (name the `/skill-name` for the human to run) rather than attempting to invoke them, since the model can't fire them from inside a subagent either way. Whenever a skill's invocation status or a flow it belongs to changes, re-check both `ask-matt` and every agent in `agents/` that references it — the same "router that lies" rule applies to agents as to `ask-matt`. To (re)link agents into the local subagent directory (`~/.claude/agents`), run `scripts/link-agents.sh`.
